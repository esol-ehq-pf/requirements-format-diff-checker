...
| IT-5 | unified diff 展開（外部 JS lazy-load、DIFF_MAX_LINES 上限） | TC-9 |
| IT-6 | セルフレビュー・実データ確認・最終コミット | TC-11 |
| IT-7 | split ビュー（左右 2ペイン・syncScrollX・max-height 窓） | RQ-16/17 |
| IT-8 | 文字レベル差分ハイライト（SequenceMatcher・#f77/#4d4） | RQ-18 |
...
| TC-9 | Different テキストファイルの HTML に外部 JS ファイル（`_diffs/dN.js`）が生成され、diff HTML が含まれること（RQ-10） | IT-5 |
| TC-10a | バイナリファイル（`.png`）が Identical / Different に正しく分類されること | IT-2 |
| TC-10b | 拡張子 `.csv` だが内容が非 UTF-8 バイナリのファイルがバイナリ扱いになること（フォールバック） | IT-2 |
| TC-11 | 実データ（input/ 配下）で比較結果が生成されること | IT-6 |
| TC-12 | stdout に `比較完了: N 件 → {out_path}` が含まれること（cli.stdout_on_success） | IT-3 |

## セルフレビュー（IT-6 で必須）

`design/output_diff_spec.json` の全 requirements（RQ-1〜RQ-18）と実装を突き合わせた表を作成し、全て OK であることを確認してからコミットする。
